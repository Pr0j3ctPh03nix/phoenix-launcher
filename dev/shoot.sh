...
SCREENS="${*:-main main:open yours setup settings:general settings:launch settings:files autoexec options confirm confirm:keep confirm:restore confirm:restore2 dest dest:flat dest:warn dest:bad gd gd:resume gd:run verify manage update whatsnew whatsnew:launcher sources files files:modified files:kept files:doomed files:onlyextras}"
